...
export default class DiagramEngine {
  ...
  handleComponentDrop = (event, component) => {
    ...
    const point = event ? getSnappedRelativeMousePoint(event) : new Point(0, 0);
  ...
  editComponentConfiguration = (node, configurations) => {
    ...
    if (hasNewPort || hasRemovedPort) {
      ...
      Object.values(portsBefore).forEach((portBefore) => {
        ...
        if (portBefore === link.getSourcePort()) link.setSourcePort(newPort);
        if (portBefore === link.getTargetPort()) link.setTargetPort(newPort);
        portBefore.remove();
      });
    }
  };

  clearSelection = () => this.getEngine().getModel().clearSelection();
  ...
  synchronizeLink = (id, value) =>
    this.getEngine().getModel().getLink(id).setValue(value);

  synchronizeComponent = (id, diff) => {
    const component = this.getEngine().getModel().getNode(id);
  ...
  clearPortValues = () =>
    this.getEngine()
      ...
      .getNodes()
      .forEach((node) =>
        Object.values(node.getPorts()).forEach((port) => port.setValue(null))
      );
}
